...
//        },
  ...
  static List<Icon> getStarsList(double rate, {double size = 18}) {
    ...
    list = List.generate(rate.floor(), (index) {
      return Icon(Icons.star, size: size, color: Color(0xFFFECD00));
    });
    if (rate - rate.floor() > 0) {
      list.add(Icon(Icons.star_half, size: size, color: Color(0xFFFECD00)));
    }
    list.addAll(List.generate(5 - rate.floor() - (rate - rate.floor()).ceil(), (index) {
      return Icon(Icons.star_border, size: size, color: Color(0xFFFECD00));
    }));
    return list;
  }
  ...
  static Widget getPrice(double myPrice, BuildContext context, {TextStyle style}) {
    ...
    try {
      if (myPrice == 0) {
        return AutoSizeText('-', style: style ?? Theme.of(context).textTheme.subtitle1);
      }
      return RichText(
        softWrap: false,
  ...
  static String getCreditCardNumber(String number) {
    ...
    else if (number != null && number.isNotEmpty && number.length == 15){
      result = number.substring(0, 4);
      result += ' ' + number.substring(4, 10);
      result += ' ' + number.substring(10, 15);
    }
    return result;
  }
