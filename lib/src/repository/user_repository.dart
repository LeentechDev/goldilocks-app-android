...
Future<User> loginSocial(String email, String password, String devicetoken) async {
  final String url = '${GlobalConfiguration().getString('api_base_url')}login';
  final client = new http.Client();
  final response = await client.post(
    url,
...
Future<User> updateUserInfo(String phone, String address) async {
  final String _apiToken = 'api_token=${currentUser.value.apiToken}';
  final String url = '${GlobalConfiguration().getString('api_base_url')}users/${currentUser.value.id}?$_apiToken';
  final client = new http.Client();
  try{
...
Future<User> updateUserPhone(String phone) async {
  final String _apiToken = 'api_token=${currentUser.value.apiToken}';
  final String url = '${GlobalConfiguration().getString('api_base_url')}users/${currentUser.value.id}?$_apiToken';
  final client = new http.Client();
  try{
...
Future<User> changepassword(User user) async {
  final String _apiToken = 'api_token=${currentUser.value.apiToken}';
  final String _userId = 'id=${currentUser.value.id}';
  final String url = '${GlobalConfiguration().getString('api_base_url')}changepassword?id=${currentUser.value.id}';
  final client = new http.Client();
 ...
 try{
  ...
  prefs.setPasswordStatus(response.body.toString());

  print("etits " + response.body.toString());
  setCurrentUser(response.body);
  currentUser.value = User.fromJSON(json.decode(response.body)['data']);
  return currentUser.value;
 }
...
Future<Stream<Address>> getAddresses() async {
  ...
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}delivery_addresses?$_apiToken&se arch=user_id:${_user.id}&searchFields=user_id:=&orderBy=updated_at&sortedBy=desc';
  try {
    final client = new http.Client();
    final streamedRest = await client.send(http.Request('get', Uri.parse(url)));
    print("test location "+ streamedRest.toString());
