...
// ValueNotifier<Restaurant> restaurant = new ValueNotifier(new Restaurant());
Future<Stream<Restaurant>> getNearRestaurants(Address myLocation, Address areaLocation) async {
  ...
  int status = await prefs.getIntOrderType();
  ...
  if (!myLocation.isUnknown() && !areaLocation.isUnknown()) {
    ...
    _queryParams['status'] = status.toString();
  }
...
Future<Stream<Restaurant>> getAllRestaurants(Address myLocation, Address areaLocation) async {
  Uri uri = Helper.getUri('api/restaurants');
  Map<String, dynamic> _queryParams = {};
  SharedPreferences prefs = await SharedPreferences.getInstance();
  Filter filter = Filter.fromJSON(json.decode(prefs.getString('filter') ?? '{}'));
