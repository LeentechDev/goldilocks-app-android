...
class FoodController extends ControllerMVC {
  ...
  void addToCart(Food food, String restaurantId, {bool reset = false}) async {
    setState(() {
      this.loadCart = true;
    });
    ...
    _newCart.restaurantID = restaurantId;
    ...
    if (_oldCart != null) {
      ...
      updateCart(_oldCart).then((value) {
        ...
        scaffoldKey?.currentState?.showSnackBar(SnackBar(
          content: AutoSizeText(S.of(context).the_food_was_added_to_cart),
        ));
      });
    } else {
  ...
  void addToFavorite(Food food) async {
    ...
    addFavorite(_favorite).then((value) {
      ...
      scaffoldKey?.currentState?.showSnackBar(SnackBar(
        content: AutoSizeText(S.of(context).thisFoodWasAddedToFavorite),
      ));
    });
  }
  ...
  void removeFromFavorite(Favorite _favorite) async {
    removeFavorite(_favorite).then((value) {
      ...
      scaffoldKey?.currentState?.showSnackBar(SnackBar(
        content: AutoSizeText(S.of(context).thisFoodWasRemovedFromFavorites),
      ));
    });
  }
